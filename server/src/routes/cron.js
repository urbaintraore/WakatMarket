/**
 * Alertes de réapprovisionnement — /api/cron/stock-alerts (protégé CRON_SECRET).
 * Port de reapprovisionnement.js : vitesse de vente journalière glissante
 * (14 jours) depuis les commandes livrées, alertes par article d'inventaire.
 *
...
export async function executerCalculVitesseEtAlertes() {
  const now = new Date();
...
router.all('/stock-alerts', async (req, res) => {
  try {
    const result = await executerCalculVitesseEtAlertes();
    res.json(result);
  } catch (error) {
    console.error('[cron] Erreur alertes stock:', error);
