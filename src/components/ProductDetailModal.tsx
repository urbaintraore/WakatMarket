...
export function ProductDetailModal({
  ...
  const full30DayHistory = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const fluctuationFactor = Math.sin((seed + i) * 0.45) * 0.05;
      const buyingPrice = Math.round(currentBuyingPrice * (1 + fluctuationFactor * 0.8));
      const sellingPrice = Math.round(currentSellingPrice * (1 + fluctuationFactor));
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto animate-fade-in">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden my-6 flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 pt-4 bg-zinc-50/50 dark:bg-zinc-950/40 border-b border-zinc-150 dark:border-zinc-800 flex items-center justify-between flex-wrap gap-3 shrink-0">
          <div className="flex p-1 bg-zinc-200/80 dark:bg-zinc-800 rounded-xl text-xs font-bold w-full sm:w-auto">
            <button
              ...
              className={`flex-1 sm:flex-none px-5 py-2 rounded-lg transition flex items-center justify-center gap-2 cursor-pointer ${
                activeTab === "stocks"
            ...
            >
              ...
              <span>Vue Stocks & Spécifications</span>
              <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${
                isLowStock ? "bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300" : "bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300"
              }`}>
                {stockValue} dispo
              </span>
            </button>
            ...
            <button
              ...
              id="tab-view-price-history"
            >
              <TrendingUp className="w-4 h-4" />
              <span>Historique des Prix (30j)</span>
              <span className="text-[10px] bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 px-1.5 py-0.2 rounded-full">
                Recharts
              </span>
            </button>
          </div>

          <div className="hidden md:flex items-center gap-2 text-xs text-zinc-500 font-medium pb-2">
            <span>Prix Actuel : <strong className="text-zinc-900 dark:text-white font-mono">{formatCFA(currentSellingPrice)}</strong></span>
          </div>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 custom-scrollbar">
          ...
          {/* TAB 1: VUE STOCKS & PARAMÈTRES D'INVENTAIRE */}
          {activeTab === "stocks" && (
              ...
              <div className="bg-zinc-50 dark:bg-zinc-950/60 p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col md:flex-row items-center gap-6">
                ...
                <div className="space-y-2 text-center md:text-left">
                  <div className="flex items-center justify-center md:justify-start gap-2">
                    ...
                    <h4 className="font-bold text-sm text-zinc-900 dark:text-white">QR Code Espace Boutique & Traçabilité</h4>
                  </div>
                  <p className="text-xs text-zinc-500 leading-relaxed">
                    Scannez ce QR code avec un smartphone ou lecteur optique pour vérifier l'authenticité et le stock de <strong className="text-zinc-800 dark:text-zinc-200">{product.name}</strong>.
                  </p>
                  <div className="flex flex-wrap gap-2 justify-center md:justify-start pt-1">
                    <button
                      onClick={() => {
                        alert(`Badge QR code pour ${product.name} prêt.`);
                      }}
          ...
          {/* TAB 2: VUE HISTORIQUE DES PRIX (RECHARTS 30 JOURS) */}
          {activeTab === "price_history" && (
            <div className="space-y-6 animate-in fade-in duration-200">
              {/* Key Pricing Cards */}
                ...
                <div className="bg-indigo-50/60 dark:bg-indigo-950/20 border border-indigo-200/80 dark:border-indigo-900/40 p-4 rounded-2xl flex flex-col justify-between">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-indigo-700/80 dark:text-indigo-400/80 mt-1 font-medium">
                      Marge moyenne: <span className="font-bold">{formatCFA(stats.avgMargin)}</span>
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-zinc-150 dark:border-zinc-800">
                  <div>
                    <h4 className="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                      ...
                      Évolution des Prix d'Achat & Vente sur {periodDays} Jours
                    </h4>
                    <p className="text-[11px] text-zinc-500">
                      Graphique interactif propulsé par Recharts avec calcul dynamique des marges journalières
                    </p>
                  </div>
        ...
        <div className="px-6 py-4 bg-zinc-50 dark:bg-zinc-950 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setActiveTab(activeTab === "stocks" ? "price_history" : "stocks")}
              className="px-3.5 py-2 rounded-xl border border-emerald-300 dark:border-emerald-800 bg-emerald-50/70 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
            >
              <ArrowUpDown className="w-3.5 h-3.5" />
              Basculer vers {activeTab === "stocks" ? "Historique des prix (Recharts)" : "Vue Stocks"}
            </button>
